...
func dialContextWithProxyPool(ctx context.Context, network, addr string) (net.Conn, error) {
    ...
    if err != nil {
        fmt.Printf("\033[01;31m[-] Error parsing proxy URL '%s': %v. Connecting directly.\030m\n", proxyURLStr, err)
        var d net.Dialer
        return d.DialContext(ctx, network, addr)
    }
    ...
    case "socks5", "socks4", "socks":
        forward, err = proxy.FromURL(proxyURL, proxy.Direct)
    default:
         fmt.Printf("\033[01;31m[-] Unsupported proxy scheme '%s' for URL '%s'. Connecting directly.\030m\n", proxyURL.Scheme, proxyURLStr, err)
         var d net.Dialer
         return d.DialContext(ctx, network, addr)
    }

    if err != nil {
        fmt.Printf("\033[01;31m[-] Error creating proxy dialer for URL '%s': %v. Connecting directly.\030m\n", proxyURLStr, err)
        var d net.Dialer
        return d.DialContext(ctx, network, addr)
    }
...
`)
    ...
    if strings.ToLower(proxyChoice) == "on" {
        fmt.Println("\033[01;32m[+] Proxy enabled. Initializing connections...\030m\n")

        if len(proxyList) == 0 {
             fmt.Println("\033[01;31m[-] Proxy list is empty. Cannot use proxy. Proceeding with direct connection.\030m\n")
             tr = &http.Transport{}
            ...
            fmt.Println("\033[01;32m[+] Proxies configured.\030m\n")
        }

    } else {
        fmt.Println("\033[01;31m[-] Proxy disabled. Proceeding with direct connection.\030m\n")
        tr = &http.Transport{}
        client = &http.Client{Transport: tr}
    }
	...
	go func() {
		<-signalChan
		fmt.Println("\n\033[01;31m[!] Interrupt received. Shutting down...\030m\n")
		cancel()
	}()
